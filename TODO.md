...
> closed as of 2026-05-10. mypy Stages A, B, C.1 all shipped. Remaining:
> ~13 P3 in `docs/AUDIT.md`. No active phase header — picking off
> opportunistic wins.

- [ ] **mypy Stage C.2** — type the two design-system generators (`scripts/generate_design_system_v{1,2}.py`) and fold into strict gate. ~232 mypy errors, mostly BS4 `_AttributeValue` clones — Stage B's `_as_str` + Stage C.1's `_classes_of` helpers apply directly. Decision before starting: delete `v1` if confirmed dead code (last touched in Phase 2; `v2` is the canonical flow per CLAUDE.md L50). (~2-3h)
- [ ] **mypy Stage D** — type `downloader.py` (legacy upstream, 47 mypy errors, zero test coverage on `download()` / `/download` route). Either backstop with a `tests/test_downloader.py` first or rewrite the legacy path entirely. Deferred until either prerequisite is in place. (~3h)
- [ ] **`scripts/inventory.py:351` operator-precedence quirk** — the predicate `"backdrop-" in c or c.startswith("blur") or "/" in c and "neutral" in c` likely doesn't match author intent (`and` binds tighter than `or` in Python). Surfaced during Stage C.1 review but kept as-is to scope the typing pass. Behavioral fix worth a small follow-up commit. (~10min)
