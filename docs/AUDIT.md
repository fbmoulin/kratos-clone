...
| P2-8 | `scripts/generate_design_system_v2.py:282-285` | **"80.8/100 DTCG coverage" is a tautology** — score computed from hardcoded `DTCG_CATEGORIES` list. Same number for any site. Fix: drive scorecard from `_inventory.json` actual contents OR reframe as "audit checklist", not "coverage score". |
...
| P2-11 | `docs/PERSONALIZATION.md` | **Prompt-injection vector unaddressed.** User `brief` interpolated into LLM prompt via f-string (`f"a {brief['category']} for ..."`). Implementation must sanitize OR use structured input not f-strings. Also: LLM HTML output should be DOM-parsed and stripped of `<script>` / `on*` / `javascript:` before write. |
