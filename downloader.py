...
class WebsiteDownloader:
    ...
    def _get_resource(self, url, base=None):
        ...
        if abs_url in self.network_resources:
            ...
            return self._save_resource(
                abs_url, res["body"], res.get("content_type", "")
            )
    ...
    def _fix_scroll_blocking(self, soup):
        ...
        if body:
            ...
            new_classes = [
                ...
                if c.lower() not in [bc.lower() for bc in blocking_classes]
            ]

            # Fix flex centering that cuts off content
            if "items-center" in new_classes and "flex" in new_classes:
                new_classes = [
                    c if c != "items-center" else "items-start" for c in new_classes
                ]
        ...
        for elem in soup.find_all(
            ...
            and any(
                x in str(c).lower()
                for x in ["scroll-container", "smooth-scroll", "lenis", "locomotive"]
            )
        ):
        ...
        for elem in soup.find_all(attrs={"style": True}):
            ...
            if "overflow" in style.lower() and "hidden" in style.lower():
                ...
                new_style = re.sub(
                    r"overflow\s*:\s*hidden\s*;?", "", style, flags=re.IGNORECASE
                )
        ...
        for script in soup.find_all("script"):
            ...
            if any(x in src.lower() for x in ["lenis", "locomotive", "smooth-scroll"]):
                ...
                x in script_text.lower()
                for x in ["new lenis", "new locomotivescroll", "smoothscroll"]
            ):
                script.decompose()
                scripts_removed += 1
    ...
    def _extract_iframe_content(self, page):
            ...
            if iframe:
                ...
                for frame in frames:
                    if (
                        ...
                        try:
                            self.log(
                                f"🔍 Detectado iframe de preview - extraindo de {frame.url[:50]}..."
                            )
                ...
                try:
                    ...
                    if frame_url == "about:srcdoc":
                        ...
                        if len(content) > 1000:  # Substantial content
                            self.log(
                                "🔍 Detectado iframe srcdoc via frame - extraindo conteúdo..."
                            )
                            return content, True
                except:
                    pass
        ...
        if body:
            ...
            if is_wrapper:
                ...
                self.log(
                    "🔍 Detectada página wrapper. Aguardando iframes internos carregarem..."
                )
    ...
    def process(self):
        with sync_playwright() as p:
            ...
            try:
                ...
                if self._is_challenge_page(page):
                    self.log(
                        "🤖 Proteção anti-bot detectada. Aguardando e tentando novamente..."
                    )
        ...
        for link in soup.find_all("link", rel="stylesheet"):
            ...
            if abs_url in self.network_resources:
                try:
                    css_content = self.network_resources[abs_url]["body"].decode(
                        "utf-8", errors="ignore"
                    )
                except:
                    pass
            ...
            if css_content:
                ...
                local_path = self._save_resource(
                    abs_url, css_content.encode("utf-8"), "text/css"
                )
                if local_path:
        ...
        for style_tag in soup.find_all("style"):
            if style_tag.string:
                style_tag.string = self._rewrite_css_urls(
                    style_tag.string, self.base_url
                )
        ...
        for meta in soup.find_all("meta", attrs={"content": True}):
            ...
            if "image" in prop.lower():
                ...
                if (
                    ...
                    and ("http" in content or content.startswith("/"))
                ):
        ...
        if is_gatsby or is_nextjs or is_nuxt:
            ...
            for script in soup.find_all("script"):
                ...
                if not is_safe:
                    ...
                    if is_nuxt and (
                        "_nuxt/" in src
                        or "__NUXT__" in script_text
                        or "nuxt" in src.lower()
                    ):
                        should_remove = True

                    # Remove inline hydration scripts
                    if (
                        "hydrate" in script_text.lower()
                        or "window.__" in script_text
                        or "GATSBY" in script_text
                        or "pageData" in script_text
                        or "self.__next" in script_text
                        or "__NEXT_DATA__" in script_text
                    ):
                        should_remove = True
            ...
            self.log(
                f"   ✅ Removidos {scripts_removed} scripts e {links_removed} preloads do framework"
            )
    ...
    def _scroll_page(self, page):
        ...
        try:
            ...
            if scroll_container:
                self.log(
                    f"🔍 Detectado container de scroll customizado: {scroll_container}"
                )
