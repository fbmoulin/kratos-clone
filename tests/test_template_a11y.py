...
def test_highlight_box_on_index(client):
    """Refinement v2: highlight box replaced by integrated #personalize-cta link.

    BETA chip + /personalize link still present, but no longer in a nested boxed
    card — flat secondary-cta pattern at the bottom of the form card.
    """
    html = client.get("/").data.decode("utf-8")
    # Old boxed card removed
    assert 'id="personalizer-highlight"' not in html
    # New integrated CTA contract
    assert 'id="personalize-cta"' in html
    assert "BETA" in html  # chip badge kept
    assert 'href="/personalize"' in html
...
# ── Refinement v2 2026-05-16: aurora, glass, integrated CTA, reveal grammar ──


def test_aurora_stage_on_both(client):
    """Aurora atmospheric layer: 3 blobs, aria-hidden, on both pages."""
    for route in ("/", "/personalize"):
        html = client.get(route).data.decode("utf-8")
        assert 'class="aurora-stage"' in html, f"{route} missing .aurora-stage"
        assert 'aria-hidden="true"' in html, f"{route} .aurora-stage not aria-hidden"
        # Exactly 3 blob divs
        assert html.count('class="aurora-blob"') == 3, (
            f"{route} should have 3 .aurora-blob, found {html.count('class="aurora-blob"')}"
        )


def test_glass_class_on_main_card(client):
    """Main card surface uses glassmorphism on both routes."""
    for route in ("/", "/personalize"):
        html = client.get(route).data.decode("utf-8")
        # Match the section/main element with id=mainCard carrying .glass
        import re

        match = re.search(r'<\w+\s[^>]*id="mainCard"[^>]*class="([^"]+)"', html) or re.search(
            r'<\w+\s[^>]*class="([^"]+)"[^>]*id="mainCard"', html
        )
        assert match, f"{route} #mainCard not found with class attribute"
        classes = match.group(1).split()
        assert "glass" in classes, f"{route} #mainCard missing .glass class (has: {classes})"


def test_tips_banner_uses_glass_light(client):
    """Tips banner is ambient (glass-light), not primary glass."""
    html = client.get("/personalize").data.decode("utf-8")
    import re

    match = re.search(r'<details\s[^>]*id="tips-banner"[^>]*class="([^"]+)"', html)
    assert match, "tips-banner not found"
    classes = match.group(1).split()
    assert "glass-light" in classes, f"tips-banner missing .glass-light (has: {classes})"
    assert (
        "glass" not in classes or "glass-light" in classes
    )  # glass-light is OK; bare glass is not


def test_integrated_cta_replaces_highlight_box(client):
    """Legacy #personalizer-highlight removed; new #personalize-cta integrated link present."""
    html = client.get("/").data.decode("utf-8")
    assert 'id="personalizer-highlight"' not in html, "legacy highlight box must be removed"
    assert 'id="personalize-cta"' in html, "new integrated CTA must be present"
    assert 'class="secondary-cta"' in html, "secondary-cta wrapper missing"
    assert 'href="/personalize"' in html, "CTA link to /personalize missing"


def test_hero_block_structure(client):
    """Hero structure: eyebrow + wordmark + tagline classes; uses <h1> for wordmark."""
    html = client.get("/").data.decode("utf-8")
    assert 'class="hero"' in html
    assert "hero-eyebrow" in html
    assert "hero-wordmark" in html
    assert "hero-tagline" in html
    # Personalize has eyebrow + wordmark (no tagline)
    p_html = client.get("/personalize").data.decode("utf-8")
    assert "hero-wordmark" in p_html


def test_reveal_classes_applied(client):
    """Refined reveal grammar: hero + main card have reveal classes for orchestrated entrance."""
    for route in ("/", "/personalize"):
        html = client.get(route).data.decode("utf-8")
        assert "reveal-d1" in html, f"{route} missing reveal-d1"
        assert "reveal-d4" in html, f"{route} missing reveal-d4 (main card delay)"


def test_reduced_motion_zeros_aurora_and_reveal(client):
    """prefers-reduced-motion guard must zero ALL the new aurora + reveal animations.

    Looks for literal rule signatures rather than nested-brace regex (CSS-nested
    blocks defeat naïve regex). Each unique string is only generated by the
    refinement v2 reduced-motion guard.
    """
    for route in ("/", "/personalize"):
        html = client.get(route).data.decode("utf-8")
        # .aurora-blob disabled under reduced-motion
        assert ".aurora-blob { animation: none" in html, (
            f"{route} reduced-motion guard does not disable .aurora-blob"
        )
        # .reveal disabled under reduced-motion
        assert ".reveal { animation: none" in html, (
            f"{route} reduced-motion guard does not disable .reveal"
        )


def test_backdrop_filter_present(client):
    """Glassmorphism contract: backdrop-filter declared in CSS."""
    for route in ("/", "/personalize"):
        html = client.get(route).data.decode("utf-8")
